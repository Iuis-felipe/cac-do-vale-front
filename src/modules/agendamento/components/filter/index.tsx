...
const Filter: React.FC<IFilterProps> = ({ setSearch, setOrder, setPage, order, handleSearch }) => {
  ...
  return (
    <div className="h-10 flex flex-row items-center">
      ...
      />
      <button
        className="h-full px-4 cursor-pointer bg-blue-700 hover:bg-blue-900 transition-colors text-white rounded-r-md"
        onClick={handleSearch}
      >
        <p className="text-sm font-semibold">Buscar</p>
      </button>
      <button
        className="border-none p-0 text-red-400 hover:text-red-500 transition-colors cursor-pointer flex flex-row items-center gap-2 ml-4"
        onClick={handleClear}
      >
      ...
      <button
        ...
        className="border-none p-0 text-blue-400 hover:text-blue-500 transition-colors cursor-pointer flex flex-row items-center gap-2 ml-4"
      >
